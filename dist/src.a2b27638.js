...
function setDelay(difficulty) {
  ...
  if (difficulty === "easy") {
    ...
    return randomInteger(600, 1200); //random number between 600 and 1200
  }
  ;
}
...
function gameOver() {
  // TODO: Write your code here
  if (time > 0) {
    var timeoutId = showUp(); // sets a different delay and a different hole
    return timeoutId;
  } else {
...
function updateTimer() {
  ...
  //When time runs out replace timer with Game Over
  else {
...
*/
function stopGame() {
  // Remove cursor class once game is over
...
*/
function startGame() {
  setEventListeners(moles); // Calling EventListeners
  //Add cursor class when start is clicked
  document.body.classList.add('cursor');
  setDuration(10);
  showUp();
  startTimer(); // Calling startTimer
  clearScore(); // Calling to reset score
  return "game started";
}
